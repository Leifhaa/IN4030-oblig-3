...
1. The prime numbers were evenly distributed among the threads to each thread would be responsible for about the same amount of prime numbers to mark
   ![alt text](docs/images/thread-responsiblity.png)
2. In each thread, the thread would check it's assigned primes and add these to a an array if it's a factor in n.
   ![alt text](docs/images/factorization-process.png)
3. After all threads were finished, they had a local array of factors which they would. The last thing we have to do is to merge these arrays and add the remainder if there is any. Then we have the complete factorization and are finished.
